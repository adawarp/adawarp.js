...
class Push {
  ...
  private matchReceive({status, response, ref}) {
...
class Socket {
  ...
  constructor(endPoint, opts: {timeout?: any, transport?: any, heartbeatIntervalMs?: any, reconnectAfterMs?: any, logger?: any, longpollerTimeout?: any, params?: any } = {}) {
  ...
  log(kind, msg, data?: any) { this.logger(kind, msg, data); }
...
class LongPoll {
  ...
  poll() {
    ...
    Ajax.request("GET", this.endpointURL(), "application/json", null, this.timeout, this.ontimeout.bind(this), (resp) => {
      let status: number, messages: any;
      if (resp) {
        let {status, token, messages} = resp;
        this.token = token;
      } else {
        status = 0;
      }
...
class Ajax {
  ...
  static serialize(obj, parentKey?: any) {
    let queryStr = [];
    for (let key in obj) {
...
export { Channel, LongPoll, Socket, Ajax }
